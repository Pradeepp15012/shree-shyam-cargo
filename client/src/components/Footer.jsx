...
const paymentOptions = ['Visa', 'Mastercard', 'RuPay', 'Net Banking', 'UPI'];
const socialLinks = [
  { label: 'Facebook', href: '#', icon: Facebook },
  { label: 'Twitter', href: '#', icon: Twitter },
  { label: 'LinkedIn', href: '#', icon: Linkedin },
  { label: 'Google Plus', href: '#', icon: Globe },
...
export default function Footer() {
  return (
    <footer className="border-t border-white/10 bg-gradient-to-br from-[#c42c23] via-[#9f1f18] to-[#7f1711] text-white">
      <div className="border-b border-white/10">
        <div className="container-shell py-10">
          <div className="rounded-[2rem] border border-white/10 bg-white/5 p-8 backdrop-blur-sm">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
              <div className="max-w-3xl">
                <p className="text-sm font-bold uppercase tracking-[0.2em] text-white/70">Need Movers Fast?</p>
            ...
            {/* <div className="mt-8">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {paymentOptions.map((item) => (
                  <span
                    key={item}
                    className="rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-xs font-bold uppercase tracking-[0.12em] text-white"
                  >
                    {item}
                  </span>
                ))}
      ...
      <div className="border-t border-white/10 bg-[#6f1510]">
        <div className="container-shell flex flex-col gap-6 py-6 xl:flex-row xl:items-center xl:justify-between">
          <div className="max-w-4xl text-sm leading-7 text-white/80">
            <p>
            ...
            <div className="flex flex-wrap items-center gap-3 xl:justify-end">
              {socialLinks.map(({ label, href, icon: Icon }) => (
                <a
                  ...
                  href={href}
                  aria-label={label}
                  className="group flex h-11 w-11 items-center justify-center rounded-full border border-white/20 bg-white/10 text-white transition hover:-translate-y-0.5 hover:bg-white/20"
                  title={label}
                >
